...
function deleteUser(req, res, next) {
  ...
  Promise.all([userPromise, eventPromise])
    ...
    .then((event, user) => {
      return res.json(event).json(user);
    });
}

router.post('/', create);
router.put('/:id([0-9A-F]{24})', update);
router.put('/:id([0-9A-F]{24})/user/:userId([0-9A-F]{24})', addUser);
router.delete('/:id([0-9A-F]{24})/user/:userId([0-9A-F]{24})', deleteUser);
router.get('/', list);
router.get('/:id([0-9A-F]{24})', getById);
router.delete('/:id([0-9A-F]{24})', destroy);
